...
Порядок моделей (нарастание сложности):
  ...
  4. Physics-Informed NN       — гибридная модель (LSTM + физиологические параметры)
"""
...
from models import TitzeBaseline, BaseLSTM, PhysicsInformedNN, InterpretableModel
...
def main():
    ...
    pinn_acc = results.get('Physics-Informed NN', {}).get('accuracy', 0)
    pinn_rec = results.get('Physics-Informed NN', {}).get('recall', 0)
